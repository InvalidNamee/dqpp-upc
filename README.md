...
- **跳转验证**：点击答题卡后检查 `.answer_list` 中是否有已选中选项——无选中项 = 新题已加载，有选中项则等待重试（最多 5 次）
- **答案收集**：考试详情页的 `<input>` 无 `value` 属性，通过解析 `.sub_result span` 的"正确答案：A"文本，结合作答时记录的字母→value 映射还原
- **题型组边界**：单选→多选、多选→判断的最后一题作答后额外等待，防止页面自动切走导致答案丢失

## 注意事项

- 需要 Chrome 浏览器和匹配版本的 chromedriver
- 登录使用 CAS 统一认证，暂不支持验证码
- `DQPP_BASE_URL` 填写根域名即可（如 `http://rdjy.upc.edu.cn`），脚本内部拼接 `/jjfz/lesson`
